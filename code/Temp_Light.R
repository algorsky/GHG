...
p.l1 = ggplot(dplyr::filter(light.2020, depth != 1)) +
  geom_line(aes(x = as.Date(Date.GMT), y = Intensity.lum.ft2, color = depth, group = depth)) +
  scale_colour_viridis_c() +
  theme_bw(base_size = 8) +
  scale_x_date(labels = date_format("%b"), breaks = "1 month", limits = as.Date(c('2019-12-01','2020-05-01')))+
  geom_vline(xintercept = as.numeric(as.Date("2020-04-26")),  linetype = "dashed", color = "black")+
...
p.l2 = ggplot(dplyr::filter(light.2021, depth != 1)) +
  ...
  ylim(0, 150)+
  # facet_wrap(vars(depth)) +
  labs(title = 'Sparkling Bog 2021') + ylab('Light(lum/ft2)') + xlab('Date') +
  NULL
...
surfaceDO<-light %>%
  ggplot() +
  geom_line(aes(x = as.Date(doy, origin = as.Date('2019-01-01')), y = Intensity.lum.ft2, color = factor(year))) +
...
ggplot(filter(light.2020, depth == 1)) +
  geom_line(aes(x = Date.GMT, y = Intensity.lum.ft2, color = '2020'), alpha = 0.7) +
  geom_line(data = filter(light.2021, depth == 1), aes(x = Date.GMT, y = Intensity.lum.ft2, color = '2021'), alpha = 0.7) +
  geom_line(data = filter(light.2019, depth == 1), aes(x = Date.GMT, y = Intensity.lum.ft2, color = '2019'), alpha = 0.7) +
  theme_bw() +
  scale_color_manual(name = 'Year',values = c('gray','lightblue4','gold')) +
  labs(title = 'Surface Sparkling Bog 2019-2021') + ylab('Light(lum/ft2)') + xlab('Date') 

ggsave('figures/SparklingBogLightString_Surf.png',width = 7, height = 3)
